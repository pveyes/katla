...
const useGameState: PersistedState<GameState> = createPersistedState('katla:gameState');
const useStats: PersistedState<GameStats> = createPersistedState('katla:gameStats');
...
export default function Home(props: Props) {
    ...
    setTimeout(() => {
      ...
      if (answer === userAnswer) {
        setStats({
          distribution: {
            ...stats.distribution,
            [gameState.attempt + 1]: stats.distribution[gameState.attempt + 1] + 1
          },
  ...
  useEffect(() => {
    ...
    function handleResize() {
      ...
      const height = (window.innerHeight - document.querySelector('#header').getBoundingClientRect().height - document.querySelector('#keyboard').getBoundingClientRect().height)
  ...
  return (
    <Container>
      ...
      <div className="mx-auto max-w-full px-4 flex justify-center items-centerg grow-0 shrink">
        ...
        <div className="grid grid-rows-6 gap-1.5 max-w-full" style={{ aspectRatio: '1 / 1' }} id="katla">
                ...
                {userAnswer.split('').map((char, index) => {
                  ...
                  return (
                    <React.Fragment key={index}>
                      <Tile key={index} char={char} state={state} isInvalid={isInvalid} delay={300 * index} />
                    </React.Fragment>
                  )
                })}
              </div>
...
export const getStaticProps: GetStaticProps<Props> = async () => {
  const { hash, date } = await fetch('https://katla.vercel.app/api/hash').then(res => res.json())
  return {
    props: {
      hash: hash,
      date: date
  ...
  return (
    <header className="px-4 mx-auto max-w-lg w-full pt-2 pb-4" id="header">
      <Head>
        ...
        <meta name="description" content="Tebak kata tersembunyi dalam 6 percobaan. Kata baru tersedia setiap hari." />

        <meta property="og:url" content="https://katla.vercel.app/" />
        <meta property="og:type" content="website" />
        <meta property="og:title" content="Katla - Permainan kata harian" />
        <meta property="og:description" content="Tebak kata tersembunyi dalam 6 percobaan. Kata baru tersedia setiap hari" />
      ...
      <div className="border-b border-b-gray-700 relative text-gray-500">
        <h1 className="uppercase text-4xl text-gray-200 font-bold w-max mx-auto relative z-10" style={{ letterSpacing: 4 }}>Katla</h1>
        <div className="absolute flex flex-row items-center justify-between inset-0">
          ...
          <div className="flex gap-2">
            <button onClick={onShowStats} title="Statistik" aria-label="Statistik">
              <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 0 24 24" width="24">
                <path fill="currentColor" d="M16,11V3H8v6H2v12h20V11H16z M10,5h4v14h-4V5z M4,11h4v8H4V11z M20,19h-4v-6h4V19z"></path>
              </svg>
            </button>
            <button onClick={onShowSetting} title="Pengaturan" aria-label="Pengaturan">
