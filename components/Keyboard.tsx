...
export default function Keyboard(props: Props) {
  const { onPressChar, onBackspace, onSubmit, gameState, answer, isAnimating } = props;
  const usedChars = new Set(
    gameState.answers
      .slice(0, gameState.attempt)
      .map(answer => answer.split('')).flat()
  )
  ...
  useEffect(() => {
    function handleKeydown(e: KeyboardEvent) {
      ...
      if (pressed.current === true && e.key === currentText[currentText.length - 1]) {
        return
      }
  ...
  return (
    <div className="max-w-lg w-full mx-auto gap-3 flex flex-col p-4" id="keyboard">
      <div className='flex gap-2'>
        {'qwertyuiop'.split('').map(char => (
          <KeyboardButton key={char} state={getKeyboardState(char)} onClick={() => onPressChar(char)}>{char}</KeyboardButton>
        ))}
      </div>
      <div className='flex gap-2'>
      ...
      <div className='flex gap-2'>
        <KeyboardButton state={null} onClick={onSubmit} scale={1.5}>Enter</KeyboardButton>
