...
export default function Tile(props: Props) {
  ...
  const border = props.char === ' '
    ? 'border'
    : props.state === null
    ? 'border-3'
    : '';
  const borderColor = props.char === ' '
    ? 'border-gray-700'
    : props.state === null
    ? 'border-gray-500'
    : '';
  ...
  return (
    <div style={style} className={`rounded-sm uppercase text-white text-center h-full w-full text-dynamic font-bold ${background} flex justify-center items-center ${border} ${borderColor}`}>
      {props.char}
    </div>
  )
}
